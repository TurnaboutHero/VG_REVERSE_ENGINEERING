...
M8/M10/M11은 가설대로 크리스탈 이후 레코드 0건(일치, 이 매치들은 애초에 미니언 mismatch가 없어 정보가 없음). M4/M6는 크리스탈 레코드가 터렛 FP라 앵커 무효(`anchor_valid=False`), M7/M9는 크리스탈 후보 자체가 없어(`crystal=None`) 앵커 무효 — 넷 다 판정 미참여로 별도 보고만 함.
