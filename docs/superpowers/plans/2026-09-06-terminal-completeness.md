...
- [x] Step 1: Add a regression for the misleading long tail before changing production. Reuse the existing signal construction and assert both uncertainty and retained approximate duration:
...
- [x] Step 3: Tighten core Step7a and add reason. For the current optional integer duration/recorded values and optional timestamp candidate/death tail, use the following decision order:
...
- [x] Step 4: Exercise v2's existing public aggregation using the uncertain long-tail signal: unknown completeness must withhold K/D/A, winner and gold from index acceptance, retain identity, and keep duration only as a withheld estimate. Preserve partial gold in safe/debug with gold_status=partial_completeness_unknown, as the existing contract does; index_export gates it via accepted_for_index=False. Follow existing test construction; do not implement a parallel policy in tests. Run targeted tests, then `python -m unittest discover -s tests`.

- [x] Step 5: Write `vg/docs/COMPLETENESS_EVIDENCE_2026-09-06.md` describing the counterexample, changed contract, sacrificed coverage, retained estimates and limitations. Link this new document from the core completeness comments if useful, leaving unrelated legacy docs untouched. Commit only task-owned files with an English `fix:` message; provide the full SHA and red/green evidence in the task report.
...
Task1 implementation: `7005bdde45a83d14864e2a3dde74f076d6b2658c`. New regression failed before implementation; targeted28/full221 passed. Task spec/quality review approved with no findings. Full60 raw-output hashes matched after removing only classification/reason. Real M6 index export withheld final stats and retained10identities; M9 remained incomplete. Core True52->26, False3->3, None5->31. V2 complete53->33, incomplete7->7, unknown0->20. Itemslot49/60 unchanged. Truth-comparison output unchanged.

Runtime evidence and remaining limitations: `vg/docs/COMPLETENESS_RUNTIME_2026-09-06.md`. The code remains on the feature branch; integration into main is outside this implementation step.
